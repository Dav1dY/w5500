...
extern uint8_t dma_tx_flag;
extern uint8_t dma_rx_flag;
...
static void w5500_spi_readburst(uint8_t* pBuf, uint16_t len)
{
  ...
  //HAL_SPI_Receive_DMA(&W5500_SPI_HANDLE, pBuf, len);
  //return;
}
...
static void w5500_spi_writeburst(uint8_t* pBuf, uint16_t len)
{
  ...
  //HAL_SPI_Transmit_DMA(&W5500_SPI_HANDLE, pBuf, len);
  //printf("state:%d\r\n",HAL_SPI_GetState(&W5500_SPI_HANDLE));
  //while(HAL_SPI_GetState(&W5500_SPI_HANDLE)!=HAL_SPI_STATE_READY);
  //return;
  //while(HAL_SPI_GetState(&W5500_SPI_HANDLE)!=HAL_SPI_STATE_READY);
  /*if((dma_tx_flag==1)&&(dma_rx_flag==1))
  {
	dma_tx_flag = 0;
	dma_rx_flag = 0;
	HAL_SPI_Transmit_DMA(&W5500_SPI_HANDLE, pBuf, len);
